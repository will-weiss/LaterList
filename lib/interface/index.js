 ...
 *
 * <ul>
 *   <li>   Familiar API: Array methods are implemented with an identical
 *          syntax. Methods that return or mutate arrays return LaterLists.
 *          Other methods wrap their return values in a Promise.
 *   </li>
 ...
 */
function LaterList() {
  this._headRef = this._tail = new Node(null);
...
LaterList.prototype.revive = function(fn, err) {
  var next, state = this._state,
    // Reference the currently pending listeners.
  ...
  while (pending) {
    // Reference the next pending listener (or null).
    next = pending.pending;
    // Clear this listener's reference to the next pending listener.
    pending.pending = null;
    // Call the specified function on the pending listener.
    fn.call(pending, err);
    // Check the next pending listener (or null).
    pending = next;
  }
  // Any listeners that were pending when revive was initially called have been
  // revived exactly once. The list may have pending listeners after revive
  // exits as listeners may fully process available nodes during this function
  // call.
};
