 ...
 *
 *   - Fully Asynchronous: A LaterList may be mapped, reduced, filtered, etc.
 *     using functions that return Promises. LaterLists process the resolved
 *     values of Promises pushed onto them.
 *
 *   - Easy Chaining: Methods of LaterLists may be chained together. Values are
 *     passed along the chain even before the original list has ended and
 *     uncaught exceptions propogate down the chain.
 *
 *   - Flexible: The two types of LaterLists - Flood and Relay - share an
 *     identical API but differ slightly in when their elements are processed.
 *     Elements of a Flood are processed as soon as they are available. Elements
 *     of a Relay are processed when processing of all prior elements is done.
 *     So, Floods have higher throughput while Relays preserve order, making
 *     each suitable for different contexts.
 *
 *   - Unbounded: LaterLists may be pushed onto indefinitely without memory
 *     overload. When a LaterList is closed, adding new listeners is disabled so
 *     that elements processed by existing listeners may be garbage collected.
 *
 *   - Active Listeners: Listeners of a LaterList are not simply callbacks; they
 *     are their own class and maintain state. As listeners keep a reference to
 *     their unprocessed elements, values may be pushed onto a LaterList more
 *     quickly than they are processed without needing to worry about
 *     backpressure for all but the most memory-intensive applications.
 *
...
LaterList.from = function(from) {
  var list = new this();
  var res = from.forEach(u.pushOne, list);
  res ? Promise.resolve(res).then(list.end, list.end) : list.end();
  return list;
};
