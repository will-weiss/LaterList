...
Listener.prototype.shift = function() {
  // Shift to and reference the next node.
  var node = this.node = this.node.next,
    // Increment and reference the index of the node.
    index = ++this.index;
  // Set the listener to be done when processing of this node and prior nodes is
  // done. First, resolve the node's value as it may be wrapped in a Promise.
  this.done = Promise.resolve(node.value).then((function(value) {
    // Process the value and index of the node with the onData function.
    return this.onData(value, index);
  // If processing of this node completed without an error, return the promise
  // indicating when all prior nodes are done processing. Uncaught exceptions
  // end this listener.
